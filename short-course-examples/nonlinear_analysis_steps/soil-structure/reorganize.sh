...
do
	...
	if [ -f $PWD/main.fei ]
	    then
	    	...
	    	# cp ${current_dir}/compare_top_acc.py ${current_dir}/"${all_dir_array[$element]}"/postprocess/
	    	cp ${current_dir}/node.fei ${current_dir}/"${all_dir_array[$element]}"/
	    	cp ${current_dir}/element.fei ${current_dir}/"${all_dir_array[$element]}"/
